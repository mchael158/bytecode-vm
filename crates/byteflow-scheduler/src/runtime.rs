...
use byteflow_bytecode::{Chunk, Value};
use byteflow_vm::{NativeTable, Vm};
...
use crate::process::{Process, ProcessId, RestartPolicy};
...
impl Runtime {
    ...
    pub fn with_natives_and_config(
        ...
        byteflow_bytecode::verify(&chunk).expect(
            "Runtime::new requires a verified chunk; call byteflow_bytecode::verify() yourself \
    ...
    /// passed [`byteflow_bytecode::verify`] (`Runtime::new` already ran it
    ...
    /// bookkeeping (worker `JoinHandle`s). Used by [`crate::supervisor::Supervisor`].
    ...
    /// A [`crate::supervisor::Supervisor`] bound to this runtime, ready to
    /// take supervised children (design notes §15).
    pub fn supervisor(&self) -> crate::supervisor::Supervisor {
        crate::supervisor::Supervisor::new(self.spawner())
    }

    /// Look up a function by name in the runtime's chunk — convenience for
    /// callers that built their chunk with [`byteflow_bytecode::ChunkBuilder`]
...
pub(crate) fn spawn_on(
    ...
    let id = crate::process::next_process_id();
...
/// [`crate::supervisor::Supervisor`] needs to respawn processes from a
...
mod tests {
    use super::*;
    use byteflow_bytecode::{ChunkBuilder, Opcode, Value};
