...
enum TimerPayload {
    ...
    /// [`crate::mailbox::Mailbox`]'s doc comment). We only hold enough to
        ...
        match entry.payload {
            ...
            TimerPayload::WakeReceiver {
                ...
                if let Some(mut process) = mailbox.take_parked() {
                    ...
                    let _ = process
                        .vm
                        .resume_with(dest_reg, byteflow_bytecode::Value::Unit);
                    injector.push(process);
                }
                // else: a `Send` already woke it; nothing to do.
            }
        }
        let _guard = notify.0.lock().unwrap_or_else(|e| e.into_inner());
        notify.1.notify_all();
    }
}
